...
public class InventoryUI : MonoBehaviour
{
    ...
    public void CreateInventory()
    {
        for (int i = 0; i < inventory.InventoryContainer.Count; i++)
        {
            ...
            obj.GetComponentInChildren<TextMeshProUGUI>().text = inventory.InventoryContainer[i].itemAmount.ToString();
    ...
    public void UpdateInventory()
    {
        for (int i = 0; i < inventory.InventoryContainer.Count; i++)
        {
            if (itemsInsideInventory.ContainsKey(inventory.InventoryContainer[i])){
                itemsInsideInventory[inventory.InventoryContainer[i]].GetComponentInChildren<TextMeshProUGUI>().text = inventory.InventoryContainer[i].itemAmount.ToString();
            }
            else
            {
                var obj = Instantiate(inventory.InventoryContainer[i].item.itemPrefab, Vector3.zero, Quaternion.identity, transform);
